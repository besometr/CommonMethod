...
//	* В модуль менеджера каждого объекта метаданных, поддерживаемого подсистемой типа, в обязательном порядке добавляются методы конструкторов.
//		* Шаблон находится в общем модуле "ОбщиеМетодыОбъектов_ШаблонМодуляМенеджера".
//		* Процедуры и функции требуют минимальной адаптации к новому объекту.
//	* Для каждого объекта метаданных, поддерживаемого подсистемой типа,  в обязательном порядке добавляется «Серверный модуль объекта».
//		* Общий модуль, наименование которого формируется по шаблону <ИмяОбъектаМетаданных>ВызовСервера, например «споГрузВызовСервера».
//		* «Серверные модули объектов» должны иметь свойства Сервер и ВызовСервера.
//		* Модуль содержит интерфейсы конструкторов в контексте Сервера.
//	* Любые вызовы конструкторов должны осуществляться через «Серверные модули объектов», 
//		например "споГрузВызовСервера.СоздатьОбъект(СтруктураОбъекта,,, ТекстОшибки);"
//	* «Серверные модули объектов» могут содержать расширенные варианты конструкторов, 
//		например "споГрузВызовСервера.СоздатьГрузСУпаковкойИПоменятьГородаМестами(СтруктураОбъекта,,, ТекстОшибки)".
//		
// Для программного создания объектов в конфигурации необходимо использовать описанные конструкторы.
//
// Описание подсистемы http://confluence/pages/viewpage.action?pageId=26346371
////////////////////////////////////////////////////////////////////////////////
